...
jQuery("#chngroup").change(function(){
	...
	if(vl) {
		if(vl == "clear") {
			jQuery("#list48").jqGrid('groupingRemove',true);
		} else {
			jQuery("#list48").jqGrid('groupingGroupBy',vl);
		}
	}
});
...
jQuery("#a3").click( function(){
	...
	var id = jQuery("#list48").jqGrid('getGridParam','selrow');
	alert(id);
	if( !id ) {
	...
	var row = jQuery("#list48").jqGrid('getRowData',id);
	/*
	//查找出yp
	//exportYy.yps
...
mydate[0].name = "test4";
	$('#list48').jqGrid().clearGridData(); 
	$('#list48').jqGrid('setGridParam', 
		{  datatype:local,  
